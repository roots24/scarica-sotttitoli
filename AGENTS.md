...
- yt-dlp has **no native txt support**: `logic` requests `.srt` via the `FFmpegSubtitlesConvertor` postprocessor and then `convert_srt_to_txt()` strips indices and timestamps into a real `.txt`, removing the `.srt`. Final file-match check is an **exact** name match (`<slug>.<lang>.<ext>`, e.g. `Titolo.en.srt`) against a pre-run snapshot of the dest folder, so stale files from previous downloads never count as success. Playlist URL = every entry processed with the same language/filter options.
